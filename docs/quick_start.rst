...
You can obtain FogLAMP in three ways:
...
- Dianomic Systems offers pre-built, certified binaries of FogLAMP for Debian using either Intel or ARM architectures. This is perhaps the simplest method for users not used to Linux. You can download the complete set of packages from https://foglamp.readthedocs.io/en/latest/92_downloads.html.
...
Once complete you can add the repository itself into the apt configuration file /etc/apt/sources.list. The simplest way to do this is the use the *add-apt-repository* command. The exact command will vary between systems;
  ...
  - Raspberry Pi does not have an apt-add-repository command, the user must edit the apt sources file manually
    ...
    .. code-block:: console
       ...
       deb  http://archives.dianomic.com/latest/buster/armv7l/ /
...
.. code-block:: console
   ...
   sudo apt -y install foglamp foglamp-gui foglamp-south-sinusoid
...
The RedHat and CentOS flavors of Linux use a different package management system, known as *yum*. FogLAMP also supports a package management system for the yum package manager.

To add the foglamp repository to the yum package manager run the command

.. code-block:: console

   sudo rpm --import http://archives.dianomic.com/RPM-GPG-KEY-foglamp

CentOS users should then create a file called foglamp.repo in the directory /etc/yum.repos.d and add the following content

.. code-block:: console

   [foglamp]
   name=foglamp Repository
   baseurl=http://archives.dianomic.com/latest/centos76/x86_64/
   enabled=1
   gpgkey=http://archives.dianomic.com/RPM-GPG-KEY-foglamp
   gpgcheck=1
...
.. code-block:: console
   ...
   [foglamp]
   name=foglamp Repository
   baseurl=http://archives.dianomic.com/latest/rhel76/x86_64/
   enabled=1
   gpgkey=http://archives.dianomic.com/RPM-GPG-KEY-foglamp
   gpgcheck=1
...
You can now install and upgrade foglamp packages using the yum command. For example to install foglamp and the foglamp GUI you run the command

.. code-block:: console

   sudo yum install -y foglamp foglamp-gui
...
The key packages to install are the FogLAMP core and the FogLAMP User Interface::
  ...
  sudo apt -y install ./foglamp-1.8.0-armv7l.deb
...
Now go to the FogLAMP user interface, create a new North instance and select the “OMF” plugin on the first screen.
...
Go to the FogLAMP user interface, create a new North instance and select the “OMF” plugin on the first screen.
The second screen will request the following information:
