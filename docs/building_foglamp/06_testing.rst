...
.. |get start| raw:: html
   ...
   <a href="building_foglamp.html" target="_blank">Building FogLAMP</a>
...
- ``=== FogLAMP services:`` - This block contains the list of microservices running in the FogLAMP platform.
...
The output of the ``foglamp status`` command gives you an idea of the modules running in your machine, but let's try to get more information from FogLAMP.
...
.. code-block:: console
  ...
  $ /usr/local/foglamp/bin/fogbench
...
.. code-block:: console
  ...
  $ /usr/local/foglamp/bin/fogbench -h
  >>> Make sure south CoAP plugin service is running & listening on specified host and port
  usage: fogbench [-h] [-v] [-k {y,yes,n,no}] -t TEMPLATE [-o OUTPUT]
                  [-I ITERATIONS] [-O OCCURRENCES] [-H HOST] [-P PORT]
                  [-i INTERVAL] [-S {total}]
...
.. code-block:: console
  ...
  $ cat /usr/local/foglamp/data/extras/fogbench/fogbench_sensor_coap.template.json
  [
...
FogLAMP uses the same *OMF Translator* plugin to send the data coming from the South modules and buffered in FogLAMP.
...
.. code-block:: console
  ...
  $ curl -s http://localhost:8081/foglamp/category | jq '.categories[] | select ( .key == "OMF to PI north" )'
  {
    "key": "OMF to PI north",
...
.. code-block:: console
  ...
  $  curl -s http://localhost:8081/foglamp/category/OMF%20to%20PI%20north | jq .plugin
  {
    "description": "PI Server North C Plugin",
    "type": "string",
    "default": "OMF",
...
.. code-block:: console
  ...
  $ curl  -X PUT http://localhost:8081/foglamp/schedule/ef8bd42b-da9f-47c4-ade8-751ce9a504be -d '{ "enabled" : true }'
  {
    "schedule": {
      "id": "ef8bd42b-da9f-47c4-ade8-751ce9a504be",
      "name": "OMF to PI north",
...
.. code-block:: console
  ...
  $ curl -s http://localhost:8081/foglamp/category/OMF%20to%20PI%20north | jq
  {
    "enable": {
      "description": "A switch that can be used to enable or disable execution of the sending process.",
      "type": "boolean",
...
.. code-block:: console
  ...
  $ curl -sH'Content-Type: application/json' -X PUT -d '{ "ServerHostname": "192.168.56.101" }' http://localhost:8081/foglamp/category/OMF%20to%20PI%20north | jq
  "ServerHostname": {
    "description": "Hostname of the server running the endpoint either PI Web API or Connector Relay",
    "type": "string",
    "default": "localhost",
